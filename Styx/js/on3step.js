    ...
    contactsent.on('click', function(e) {
      ...
      if (0 == a.length || "-1" == a.indexOf("@") || "-1" == a.indexOf(".")) {
        ...
        contactemail.css({
          "border-top": "none",
          "border-left": "none",
          "border-bottom": "1px solid C1872A",
          "border-right": "none"
        });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function showPopup(contentClass) {
    console.log(screen.width);
    const content = document.querySelector(contentClass);
    if (content && screen.width<=985) {
  ...
  function hidePopup() {
    popup.classList.remove("mystyle");
  }

  // Event listener for the close button
  closeButton.addEventListener('click', hidePopup);

  // Event listener for step menu items
  document.querySelectorAll('.filt-step').forEach(item => {
